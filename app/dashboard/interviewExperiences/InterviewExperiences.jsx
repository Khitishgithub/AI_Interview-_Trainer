...
function Avatar({ name, size = "lg" }) {
  ...
  return (
    <div
      className={`${cls} bg-gradient-to-br ${pick[0]} flex items-center justify-center font-black text-white shadow-lg ${pick[1]} shrink-0`}
    >
      {initials}
    </div>
  );
      ...
      >
        ...
        <div className="flex items-start gap-4 mb-5">
          ...
          <div className="min-w-0">
            <h2 className="text-white font-black text-base group-hover:text-indigo-300 transition-colors truncate">
              {item.name}
            </h2>
            <div
              className={`inline-flex items-center gap-1.5 mt-1 px-2.5 py-1 rounded-lg border text-[10px] font-bold uppercase tracking-widest ${co.bg} ${co.border} ${co.color}`}
            >
              {item.company}
            </div>
            {item.role && (
              <p className="text-white text-[11px] mt-1 truncate ">
                {item.role}
              </p>
            )}
          </div>
        ...
        {/* Quote */}
        <p className="text-white text-xs leading-relaxed line-clamp-3 mb-5 font-sans italic border-l-2 border-indigo-500/30 pl-3">
          "{item.shortQuote}"
        </p>
        ...
        <div className="flex flex-wrap gap-2 mb-5">
          {item.date && (
            <span className="flex items-center gap-1 text-[9px] px-2 py-1 rounded bg-white/4 text-white border border-white/6 font-bold uppercase">
              <Calendar size={8} />
              {item.date}
            </span>
          )}
    ...
    <div className="relative flex gap-5">
      ...
      <div className="pb-6 min-w-0">
        <div className="flex items-center gap-2 flex-wrap mb-1">
          <h4 className="text-white font-black text-sm">{round.round}</h4>
          {round.duration && (
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundImage: `linear-gradient(90deg, #4f46e5, #7c3aed, #db2777)`,
          }}
        />
        ...
        <div className="px-8 py-6 border-b border-white/6 flex items-start justify-between gap-4 shrink-0 bg-[#07070f]">
          <div className="flex items-start gap-5">
            ...
            <div>
              <h2 className="text-white text-2xl font-black">{item.name}</h2>
              <div className="flex flex-wrap items-center gap-2 mt-2">
                <span
                  className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[10px] font-bold uppercase tracking-widest ${co.bg} ${co.border} ${co.color}`}
                >
                  {item.company}
                </span>
                {item.role && (
                  <span className="text-white text-[11px] ">{item.role}</span>
                )}
              </div>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-2 rounded-xl bg-white/4 hover:bg-white/8 text-white hover:text-white transition-colors shrink-0 border border-white/6"
          >
            <X size={18} />
          </button>
        </div>
        ...
        {/* Scrollable body */}
        <div
          className="overflow-y-auto flex-1 px-8 py-8 space-y-10"
          style={{
            scrollbarWidth: "thin",
          ...
          <div className="grid md:grid-cols-3 gap-10">
            ...
            <div className="md:col-span-2 space-y-10">
              ...
              {item.overview && (
                <section>
                  ...
                  />
                  <div className="mt-4 p-5 rounded-2xl bg-white/[0.02] border border-white/6 text-white text-sm leading-relaxed font-sans">
                    {item.overview}
                  </div>
                </section>
              )}
            ...
            <div className="space-y-5">
              ...
              <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/6 space-y-4">
                <h3 className="text-[9px] font-bold uppercase tracking-[0.2em] text-white">
                  Quick Info
                </h3>
                {item.location && (
                  <InfoRow
                ...
                {item.timeline?.length && (
                  <InfoRow
                    icon={Users}
                    label="Rounds"
                    value={`${item.timeline.length} interviews`}
                  />
                )}
              </div>
                ...
                <div className="p-5 rounded-2xl border border-indigo-500/20 bg-indigo-500/5 space-y-4">
                  ...
                  <ul className="space-y-3">
                    {item.tips.map((tip, i) => (
                      <li
                        key={i}
                        className="flex gap-2.5 text-white text-xs leading-relaxed font-sans"
                      >
              ...
              {item.outcome && (
                ...
                >
                  ...
                  <div>
                    ...
                    <p
                      className={`text-sm font-black ${item.outcome === "Offer" || item.outcome === "Selected" ? "text-emerald-300" : "text-rose-300"}`}
                    >
                      {item.outcome}
                    </p>
                  </div>
...
function SectionTitle({ icon: Icon, label, color }) {
  return (
    <div className="flex items-center gap-3">
      <div
        className={`p-1.5 rounded-lg bg-white/4 border border-white/6 ${color}`}
      >
        <Icon size={14} />
      </div>
...
function InfoRow({ icon: Icon, label, value }) {
  return (
    <div className="flex items-start gap-2.5">
      <Icon size={12} className="text-white mt-0.5 shrink-0" />
      <div>
        <p className="text-[9px] uppercase tracking-widest text-white font-bold">
          {label}
        </p>
