...
import {
  Play, CheckCircle, XCircle, Loader2, ChevronLeft,
  BookOpen, Lightbulb, Terminal, RotateCcw, Copy, Check,
} from "lucide-react";
...
export default function QuestionPage() {
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        setQuestion(q);
        setLanguage(q.language && LANGUAGE_IDS[q.language] ? q.language : "C++");
  ...
  const runCode = useCallback(async () => {
    ...
    if (!rapidApiKey) {
      ...
      setResult({
        status: "simulated",
        stdout: "⚠️ Add NEXT_PUBLIC_RAPIDAPI_KEY to .env.local to enable real execution.\n\nCode received successfully!",
        time: "~", memory: "~",
      });
      setRunning(false);
      return;
    ...
    try {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (data.status?.id > 2) { // not queued/processing
          output = data;
          break;
        }
      }
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-white  flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 bg-[#0d0d14]">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-semibold text-white truncate max-w-xs">{question.title}</span>
          <span className={`text-[10px] px-2 py-0.5 rounded-full border ${diffColors[question.difficulty] || ""}`}>
            {question.difficulty}
          </span>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 text-sm">
            ...
            {activeTab === "description" && (
              <div className="space-y-5">
                <div>
                  <h1 className="text-lg font-bold text-white mb-1">{question.title}</h1>
                  <div className="flex gap-2 flex-wrap">
                    <span className="text-[10px] text-gray-500 border border-white/5 px-2 py-0.5 rounded-full">
                      {question.platform}
                    </span>
                    {question.tags?.map(t => (
                      <span key={t} className="text-[10px] text-gray-600 border border-white/5 px-2 py-0.5 rounded-full">
                        {t}
                      </span>
                    ))}
                  </div>
                ...
                <p className="text-gray-300 leading-relaxed text-xs">{question.description}</p>

                {question.examples?.length > 0 && (
                  <div className="space-y-3">
                    <h3 className="text-xs uppercase tracking-widest text-gray-600">Examples</h3>
                    {question.examples.map((ex, i) => (
                      <div key={i} className="bg-white/[0.03] border border-white/5 rounded-xl p-4 text-xs space-y-2">
                        <div><span className="text-gray-500">Input: </span><span className="text-indigo-300">{ex.input}</span></div>
                ...
                {question.constraints?.length > 0 && (
                  <div>
                    <h3 className="text-xs uppercase tracking-widest text-gray-600 mb-2">Constraints</h3>
                ...
                {question.testCases?.length > 0 && (
                  <div>
                    <h3 className="text-xs uppercase tracking-widest text-gray-600 mb-2">Test Cases</h3>
                    <div className="space-y-2">
                      {question.testCases.map((tc, i) => (
                        <div key={i} className="bg-white/[0.02] border border-white/5 rounded-lg p-3 text-xs">
                          <div><span className="text-gray-500">In: </span><span className="text-blue-300">{tc.input}</span></div>
                          <div><span className="text-gray-500">Expected: </span><span className="text-emerald-300">{tc.expectedOutput}</span></div>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            {activeTab === "solution" && (
              <div className="space-y-4">
                <div className="bg-amber-400/10 border border-amber-400/20 rounded-xl p-4 text-xs text-amber-300">
                  Try solving it yourself first! The solution is hidden until you reveal it.
                </div>
                {!showSolution ? (
                  ...
                  <div>
                    <p className="text-xs text-gray-500 mb-2">Solution in {language}:</p>
                    <pre className="bg-white/[0.03] border border-white/5 rounded-xl p-4 text-xs text-emerald-300 overflow-x-auto whitespace-pre-wrap">
                      {question.solution?.[language] || "No solution available for this language."}
                    </pre>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                {!result && !running && (
                  <div className="flex flex-col items-center justify-center py-16 text-center">
                    ...
                    <p className="text-gray-600 text-xs">Run your code to see output</p>
                  </div>
                )}
                {running && (
                  <div className="flex items-center gap-2 text-indigo-400 text-xs py-8 justify-center">
                    <Loader2 size={16} className="animate-spin" /> Executing code...
                  </div>
                )}
                {result && !running && (
                  <div className="space-y-4">
                    ...
                    <div className={`flex items-center gap-2 px-4 py-3 rounded-xl border text-sm font-semibold ${
                      ...
                      {result.passed
                        ...
                        ? <Terminal size={16} />
                        : <XCircle size={16} />}
                      {result.status === "simulated" ? "Simulation Mode" : result.passed ? "Accepted" : result.status}
                    ...
                    {result.stdout && (
                      <div>
                        <p className="text-xs text-gray-600 mb-1 uppercase tracking-widest">Output</p>
                    ...
                    {result.expected && (
                      <div>
                        <p className="text-xs text-gray-600 mb-1 uppercase tracking-widest">Expected</p>
                    ...
                    {result.stderr && (
                      <div>
                        <p className="text-xs text-gray-600 mb-1 uppercase tracking-widest">Error</p>
